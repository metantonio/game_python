...
class Enemy(Entity):
    def __init__(self, monster_name, pos, groups, obstacle_sprites, damage_player, trigger_death_particles):
        ...
        self.status = 'idle'
        #self.image = pygame.Surface((64,64)) #generic for status
        self.image = self.animations[self.status][self.frame_index] #frame_index is generic from Entity class, but could be changed
    ...
    def get_player_distance_direction(self, player):
        enemy_vector = pygame.math.Vector2(self.rect.center) ##working with the center of enemy position
        ...
        if distance > 0:
            ...
            direction = pygame.math.Vector2() #default vector is (0,0)
    ...
    def get_damage(self, player, attack_type):
        # Needs a timer to hit the enemy one cycle at time
        if self.vulnerable:
            self.direction = self.get_player_distance_direction(player)[1]
